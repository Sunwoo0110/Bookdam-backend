...
public class BookCreateReqDto {
    @Schema(description = "도서명 (필수, 최대 128자)", example = "토비의 스프링 3.1")
    @NotBlank
    @Size(max = 128)
    private String title;

    @Schema(description = "저자명 (필수, 최대 64자)", example = "이일민")
    @NotBlank
    @Size(max = 64)
    private String author;

    @Schema(description = "ISBN (필수, 10자리 또는 13자리)", example = "9788960774041")
    @NotBlank
    @Pattern(
            regexp = "^(?:\\d{9}[\\dX]|\\d{13})$",
    ...
    @Schema(description = "설명 (optional, 최대 2000자)", example = "스프링 핵심 가이드")
    @Size(max = 2000)
